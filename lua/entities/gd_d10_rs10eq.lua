...
function ENT:Initialize()	
	if (SERVER) then
		...
		self.Child = createEarthquake(10, self)
		self:Explode()
	end
end
...
function ENT:Explode()
	...
	for k, v in pairs(ents.GetAll()) do 
		...
		if phys:IsValid()  and  (v:GetClass()!= "phys_constraintsystem" and v:GetClass()!= "phys_constraint"  and v:GetClass()!= "logic_collision_pair") and v:IsSolid() then
			timer.Simple(t, function()
				if !v:IsValid() or !phys:IsValid() then return end 
				if v:IsNPC() or v:IsPlayer() and v:IsOnGround() then 
					v:SetVelocity( Vector(0,0,600)) 
				else
					phys:AddVelocity( Vector(0,0,400) )
				end
			end)
		end
	end
