...
function ENT:Smite()
	...
	timer.Simple(0.05, function()
		...
		if (target:IsValid() and self:IsValid()) and explosion != ""  then ParticleEffect(explosion, target:GetPos(), Angle(0,0,0), nil) end 

		
		if (target:IsValid() and self:IsValid()) then CreateSoundWave(table.Random(sounds), target:GetPos(), "3d" ,340/2, {70,110}, 0.5) end
