...
function ENT:GetEntitiesInsideLava()
	...
	local lents = {} 
	for k, v in pairs(ents.FindInSphere(self:GetLavaLevelPosition() - Vector(0,0,200), 180)) do
		local pos = v:GetPos()
...
function ENT:Erupt()
	...
	timer.Simple(2, function() -- we have a delay here because air is still expanding from heat
		if !self:IsValid() then return end
		CreateSoundWave("streams/disasters/nature/minivolcano_explosion.mp3", self:GetPos(), "stereo" ,340.29/2, {100,100}, 5)
	end)
	self:CreateRocks( 5, {16,20} )
