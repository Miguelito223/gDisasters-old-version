...
function ENT:Initialize()	
	...
	timer.Simple(1, function()
		...
		local sound = Sound("streams/environment/water_shared/water_whirlpool_loop_01.wav")
...
function ENT:Vortex()
	...
	local entities = FindInCylinder(self:GetPos(), 720, 2,  -200, true)
	...
	for k, v in pairs(entities) do
		if v:GetPhysicsObject():IsValid() then
			...
			local vert_force   = (math.Clamp(720 / twoDDistance^2,0,1) * 90) * Vector(0,0,-1)
			local horiz_force  = (math.Clamp(720 / twoDDistance^1.1,0,1) * 66) * dir2_tangent
			
			if v:IsPlayer() or v:IsNPC() then
				v:SetVelocity( dir * 70 + (vert_force*7) + (horiz_force*0.2))

			else
				phys:AddVelocity( dir * 60 + (vert_force*5) + horiz_force)
