...
ENT.PrintName		                 =  "Light Rain ENTITY"
ENT.Author			                 =  "Hmm"
ENT.Contact		                     =  "Hmm"
ENT.Category                         =  "Hmm"
...
function ENT:Initialize()		
	...
	if (SERVER) then
		...
		GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(10,20),["Direction"]=Vector(-1,0,0)}, ["Pressure"]    = 95000, ["Temperature"] = math.random(12,21), ["Humidity"]    = math.random(40,85), ["BRadiation"]  = 0.1}}
...
function ENT:AffectPlayers()
	...
	for k, v in pairs(player.GetAll()) do
		...
		if v.gDisasters.Area.IsOutdoor then
			...
			net.WriteString("localized_light_rain_effect", Angle(0,math.random(1,20),0))
			net.Send(v)
			...
			if math.random(1,2) == 1 then
				...
				net.WriteFloat(math.random(5,158))
...
function ENT:Think()
	if (CLIENT) then
		...
		if math.random(1,2800)==1 then surface.PlaySound("streams/disasters/nature/thunder"..tostring(math.random(1,3))..".wav") end 
